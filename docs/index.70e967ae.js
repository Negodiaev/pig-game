...
function rollDice() {
    ...
    } else // reset the current score and switch a player
    switchPlayer();
}
function hold() {
    ...
    if (isActivePlayerFirst) totalScore1 += currentScore;
    else totalScore2 += currentScore;
    ...
    if (totalScore < 100) // reset the current score and switch a player
    switchPlayer();
    else {
...
function getScoreEl(type) {
    if (type === "current") return document.getElementById(`current-${activePlayer}`);
    else return document.getElementById(`player-total-${activePlayer}`);
}
function hideDiceEl() {
